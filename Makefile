.PHONY: help makehelp dev server daemon cli multica build test migrate-up migrate-down migration-lint migration-new sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree remove-worktree db-up db-down db-drop db-reset selfhost selfhost-build selfhost-stop up down status list destroy gc env-exec api-dev web-dev desktop-dev
...
migration-lint: ## Check migration numbering (no new duplicate prefixes) and up/down pairing; needs no database
	cd server && go test ./internal/migrations -run '^TestMigration' -count=1

migration-new: ## Allocate the next migration number from origin/kun and create the empty up/down pair (needs NAME=...)
	bash scripts/migration-new.sh "$(NAME)"
