...
const countConsecutiveFailureFingerprint = `-- name: CountConsecutiveFailureFingerprint :one
SELECT COUNT(*)::bigint
FROM (
    SELECT failure_fingerprint
    FROM agent_task_queue
    WHERE failure_input_version = $1
      AND status = 'failed'
      AND failure_fingerprint IS NOT NULL
    ORDER BY completed_at DESC NULLS LAST, id DESC
    LIMIT 2
) recent
WHERE failure_fingerprint = $2
`

type CountConsecutiveFailureFingerprintParams struct {
	FailureInputVersion pgtype.Text `json:"failure_input_version"`
	FailureFingerprint  pgtype.Text `json:"failure_fingerprint"`
}

func (q *Queries) CountConsecutiveFailureFingerprint(ctx context.Context, arg CountConsecutiveFailureFingerprintParams) (int64, error) {
	row := q.db.QueryRow(ctx, countConsecutiveFailureFingerprint, arg.FailureInputVersion, arg.FailureFingerprint)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}
...
SET status = 'failed',
    ...
    failure_input_version = $4,
    failure_fingerprint = $5,
    session_id = CASE WHEN $6 THEN NULL ELSE COALESCE($7, session_id) END,
    work_dir = COALESCE($8, work_dir),
    durable_work_dir = COALESCE($9, durable_work_dir),
    branch_name = COALESCE($10, branch_name),
    session_rollout_missing = $6,
...
const updateAgentPlanLimits = `-- name: UpdateAgentPlanLimits :execrows
UPDATE agent
SET plan_limits = $1
WHERE id = $2
  AND workspace_id = $3
...
type UpdateAgentPlanLimitsParams struct {
...
func (q *Queries) UpdateAgentPlanLimits(ctx context.Context, arg UpdateAgentPlanLimitsParams) (int64, error) {
	result, err := q.db.Exec(ctx, updateAgentPlanLimits, arg.PlanLimits, arg.ID, arg.WorkspaceID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
