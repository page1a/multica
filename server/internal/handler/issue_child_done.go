...
// resolveTerminalChildren is the predicate-shaped compatibility form used by
// older callers. New notification paths keep the full resolved snapshot so
// comment builders can distinguish done from cancelled.
func resolveTerminalChildren(children []db.Issue, effective func(db.Issue) (string, error)) (func(db.Issue) bool, error) {
	statuses, err := resolveChildStatuses(children, effective)
	if err != nil {
		return nil, err
	}
	return statuses.isTerminal, nil
}
