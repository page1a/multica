...
func (h *Handler) enqueueSingleCommentTrigger(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, trigger commentAgentTrigger) error {
	...
	case commentTriggerSourceIssueAssignee:
		if trigger.Squad != nil {
			if _, err := h.enqueueSquadLeaderCommentTask(ctx, issue, trigger, triggerCommentID, service.OriginDerived); err != nil {
	...
	case commentTriggerSourceMentionSquadLeader:
		if _, err := h.enqueueSquadLeaderCommentTask(ctx, issue, trigger, triggerCommentID, service.OriginNamed); err != nil {
	...
	case commentTriggerSourceMentionAgent:
		if _, err := h.enqueueMentionCommentTask(ctx, issue, trigger, triggerCommentID, service.OriginNamed); err != nil {
	...
	case commentTriggerSourceThreadParent, commentTriggerSourceConversation:
		...
		if trigger.Squad != nil {
			_, err = h.enqueueSquadLeaderCommentTask(ctx, issue, trigger, triggerCommentID, service.OriginNamed)
		} else {
